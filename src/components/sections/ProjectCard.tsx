  ...
  return (
    <ScrollReveal delay={index * 100} direction="up">
      <Card3D className="w-full">
        <article className="group relative bg-slate-800/50 backdrop-blur-sm rounded-2xl overflow-hidden border-2 border-slate-700/50 hover:border-blue-500/50 transition-all duration-500">
          ...
          <div className="relative flex flex-col lg:flex-row">
            ...
            <div className="relative lg:w-2/5 h-64 lg:h-auto lg:min-h-[320px] bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 flex items-center justify-center overflow-hidden">
              {project.imageUrl ? (
                <Image
                ...
                <div className="relative w-full h-full flex items-center justify-center">
                  <div className="text-[120px] font-bold text-blue-500/20 group-hover:scale-110 transition-transform duration-500" aria-hidden="true">
              ...
              {medal && (
                <div className="absolute top-4 left-4 z-10">
                  <div className={`flex items-center gap-2 px-4 py-2 bg-gradient-to-r ${medal.gradient} text-white font-bold rounded-full shadow-lg`}>
                    <span className="text-2xl">{medal.emoji}</span>
                    <span className="text-sm">{medal.text}</span>
                  </div>
                </div>
              )}
              ...
              {project.isPrivate && (
                <div className="absolute top-4 right-4 z-10">
                  <span className="px-3 py-1 bg-rose-500/90 text-white text-xs font-medium rounded-full shadow-lg">
                    Private
                  </span>
                </div>
              )}
            ...
            <div className="relative lg:w-3/5 p-8">
              {/* Title and stars */}
              ...
              {/* Description */}
              <p className="text-zinc-300 text-lg mb-6 leading-relaxed line-clamp-4">
                {project.aiSummary || project.description}
              </p>

              {/* Technologies */}
              <div className="flex flex-wrap gap-2 mb-6">
                {project.technologies.split(',').slice(0, 8).map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 text-sm font-medium bg-slate-700/50 text-zinc-200 rounded-lg border border-slate-600/50"
                  >
                    {tech.trim()}
                  </span>
                ))}
                {project.technologies.split(',').length > 8 && (
                  <span className="px-3 py-1.5 text-sm font-medium bg-slate-700/50 text-zinc-400 rounded-lg border border-slate-600/50">
                    +{project.technologies.split(',').length - 8}
                  </span>
                )}
              </div>
                ...
                <div className="flex items-center gap-3">
                  {project.repoUrl && (
                    <a
                      ...
                      className="flex items-center gap-2 px-5 py-2.5 bg-slate-700/50 text-zinc-200 rounded-xl hover:bg-slate-600/50 transition-all duration-300 text-sm font-medium border border-slate-600/50 hover:border-slate-500/50"
                    >
                  ...
                  {project.demoUrl && (
                    <a
                      ...
                      className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-300 text-sm font-medium"
                    >
