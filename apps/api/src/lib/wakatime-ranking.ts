// WakaTime Year in Review Ranking Extractor. Ported verbatim from the web app's
// src/lib/wakatime-ranking.ts (imports identical at the lib root).
...
/** Extract ranking information from a WakaTime Year in Review page. */
...
- Return ONLY valid JSON, no other text`;
    ...
    if (typeof data.percentile !== 'number' || data.percentile <= 0) return null;
    ...
    return {
      ...
      bestDay: data.bestDayDate ? { date: data.bestDayDate, hours: data.bestDayHours || '' } : undefined,
    };
  } catch (error) {
    success = false;
  ...
  for (const pattern of rankingPatterns) {
    ...
    if (match) {
      ...
      if (percentile > 0 && percentile <= 100) {
        ...
        const totalHours = hoursMatch
          ? `${hoursMatch[1]} hrs${hoursMatch[2] ? ` ${hoursMatch[2]} mins` : ''}`
          : undefined;

        const avgMatch = html.match(
          /(?:daily\s+average|average\s+daily)[:\s]*(\d+)\s*(?:hrs?|hours?)\s*(?:(\d+)\s*(?:mins?|minutes?))?/i
        );
        const dailyAverage = avgMatch
          ? `${avgMatch[1]} hrs${avgMatch[2] ? ` ${avgMatch[2]} mins` : ''}`
          : undefined;
...
export async function extractRankingsFromUrls(
  ...
  for (const [yearStr, url] of Object.entries(yearlyReportLinks)) {
    ...
    if (ranking) rankings[year] = ranking;
  }
